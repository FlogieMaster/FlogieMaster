...
let dictionarySettings = JSON.parse(localStorage.getItem('dictionarySettings')) || { "Color": "white", "Backgroundcolor": "black", "clocksettings": [0, 0, 1, 1] };
...
function clock() {
    ...
    for (let i = 0; i < 4; i++) {
        if (dictionarySettings["clocksettings"][i] == 1) {
            ...
            if (i == 0 && dictionarySettings["clocksettings"][i+2] == 1 && dictionarySettings["clocksettings"][i+1] == 0) time += "  ";
        }
...
function buttonscolor() {
    for (let i = 0; i < 4; i++) {
        if (dictionarySettings["clocksettings"][i] == 1) {
            document.querySelector(clockids[i]).style.backgroundColor = "var(--maincolor)";
            document.querySelector(clockids[i]).style.color = "var(--backgroundcolor)";
        } else {
            document.querySelector(clockids[i]).style.backgroundColor = "var(--backgroundcolor)";
...
function buttonclocksettingsclicked(id) {
    (id % 2 == 0) ?
        (dictionarySettings["clocksettings"][id] == 0) || (dictionarySettings["clocksettings"][id + 1] = 0) :
        (dictionarySettings["clocksettings"][id] == 1) || (dictionarySettings["clocksettings"][id - 1] = 1);
...
function gotosettings() {
    body.innerHTML = '<div class="position"><svg xmlns="http://www.w3.org/2000/svg" onclick="onstart();" width="48" height="48" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-width="1.5"><pathd="M2 12c0-4.714 0-7.071 1.464-8.536C4.93 2 7.286 2 12 2c4.714 0 7.071 0 8.535 1.464C22 4.93227.286 22 12c0 4.714 0 7.071-1.465 8.535C19.072 22 16.714 22 12 22s-7.071 0-8.536-1.465C2 19.072216.714 2 12Z" /><path stroke-linecap="round" d="m14.5 9.5l-5 5m0-5l5 5" /></g></svg></div><div class="settings"><span class="setting-title">Settings</span><div class="forms"><form onchange="settings_maincolor()" class="color_I"><input type="color" id="main_color_input"></form><form onchange="settings_backgroundcolor()" class="color_I"><input type="color" id="background_color_input"></form></div><div class="clock_settings"><div class="clockinputs"><button id="day" onclick="buttonclocksettingsclicked(0);">day.month</button><button id="year" onclick="buttonclocksettingsclicked(1);">.year</button><button id="hour" onclick="buttonclocksettingsclicked(2);">hour:minute</button><button id="second" onclick="buttonclocksettingsclicked(3);">:second</button></div></div><div class="addons"><div class="add_addons"><div class="add_text">+</div></div></div></div>';
